...
class SyncLoRAPlanner:
    ...
    支持Loading LoRA 权重（PeftModel），可选 thinking、右半裁剪
    """
    ...
    def __init__(
        ...
        if lora_path and lora_path.lower() != "none":
            ...
            logging.info(f"Loading LoRA 权重: {lora_path}")
            self.model = PeftModel.from_pretrained(self.model, lora_path)
            logging.info("LoRA loaded！")

        self.model.eval()
        logging.info(f"模型loaded！(thinking={enable_thinking}, crop_right_half={crop_right_half})")
